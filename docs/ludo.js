			...
			[
				...
				function (room) {
					return A3(
						...
						room,
						1,
						A2($elm$core$Maybe$withDefault, 2, model.E));
				})
			]));
};
...
var $author$project$Ludo$canMove = F2(
	function (model, posInfo) {
		...
		var pos = _v0.b;
		return (!(!model.H)) && (_Utils_eq(model.aU, playerColor) && function () {
			switch (pos.$) {
				case 1:
					return model.H === 6;
				case 2:
					var num = pos.b;
					return !((num + model.H) > 6);
				default:
					return !(!model.H);
			}
		}());
...
var $author$project$Ludo$move = F3(
	function (posInfo, model, clickedPosition) {
		...
		while (true) {
			...
			if ((!_Utils_eq(model.aU, color)) || (!model.H)) {
				return posInfo;
			} else {
	...
	function (clickedPosition, model) {
		...
		return _Utils_update(
			model,
			{
				H: 0,
				aK: updatedPos,
				aU: ((model.H === 6) || (A3($author$project$Ludo$killHappened, model.aU, model.aK, updatedPos) || A3($author$project$Ludo$goalPathReached, model.aU, model.aK, updatedPos))) ? model.aU : A2($author$project$Ludo$nextTurn, model, model.aU)
			});
	});
...
var $author$project$Dice$diceDiv = function (model) {
	...
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('disabled:opacity-50 rounded-full hover:bg-gray-600 focus:outline-none focus:shadow-outline col-span-2 row-span-2 border p-2 m-2 ' + positionClass),
				((!model.H) && _Utils_eq(model.aU, model.aN)) ? $elm$html$Html$Events$onClick($author$project$LudoModel$RollDice) : $elm$html$Html$Attributes$hidden(false)
			]),
		_List_fromArray(
			[
				$elm$html$Html$text(
				(!model.H) ? 'roll' : $elm$core$String$fromInt(model.H))
			]));
};
